...
public interface SeoulMapper {
	 ...
	 * parameterType="hashmap" > SELECT no,contentid,title,address,image1,hit FROM
	 * seoultravel WHERE contenttype=#{contenttype} ORDER BY no ASC OFFSET #{start}
	 * ROWS FETCH NEXT 12 ROWS ONLY </select>
	 */
	 ...
	 * infocenter,restdate,usetime,parking,msg FROM seoultravel s JOIN attraction a
	 * ON s.contentid=a.contentid AND s.contentid=#{contentid} </select>
	 */
	public SeoulVO seoulAttractionDetailData(int contentid);

	/*
	 * <update id="seoulHitIncrement" parameterType="int"> UPDATE seoultravel SET
	 * hit=hit+1 WHERE contentid=#{contentid} </update>
	 */
	public void seoulHitIncrement(int contentid);

	/*
	 * <!-- 검색 --> <select id="seoulFindData" resultType="com.sist.web.vo.SeoulVO"
	 * parameterType="hashmap" > SELECT
	 * no,contentid,title,address,image1,hit,contenttype FROM seoultravel WHERE
	 * address LIKE '%'||#{address}||'%' ORDER BY no ASC OFFSET #{start} ROWS FETCH
	 * NEXT 12 ROWS ONLY </select> <select id="seoulFindTotalPage" resultType="int"
	 * parameterType="string" > SELECT CEIL(COUNT(*)/12.0) FROM seoultravel WHERE
	 * address LIKE '%'||#{address}||'%' </select>
	 */
	...
	/*
	 * <select id="seoulTop5Data" resultType="com.sist.web.vo.SeoulVO"> SELECT
	 * no,contentid,title,address,image1,hit,rownum FROM (SELECT
	 * no,contentid,title,address,image1,hit FROM seoultravel WHERE contenttype=39
	 * ORDER BY hit DESC) WHERE rownum&lt;=5 </select>
	 */
	public List<SeoulVO> seoulTop5Data();

	/*
	 * <!-- private int no,contentid; private String
	 * eventstartdate,eventenddate,agelimit,
	 * playtime,eventplace,eventhomepage,usetime,spendtime,msg;
	 * 
	 * image1,x,y,title,address -->
	 */
	@Results({ @Result(property = "fvo.eventstartdate", column = "eventstartdate"),
			...
			@Result(property = "fvo.spendtime", column = "spendtime"), @Result(property = "fvo.msg", column = "msg") })
	@Select("SELECT f.no,image1,x,y,title,address," + "eventstartdate,eventenddate,agelimit,"
			+ "playtime,eventplace,eventhomepage,usetime,spendtime,msg " + "FROM seoultravel s, festival f "
			+ "WHERE s.contentid=f.contentid " + "AND s.contentid=#{contentid}")
	public SeoulVO seoulFestvalDetailData(int contentid);

	
	@Results({ @Result(property = "fsvo.firstmenu", column = "firstmenu"),
