...
public interface ReserveMapper {
	/*
	 * @Select("SELECT no,contentid,image1,title,x,y,address " + "FROM seoultravel "
	 * + "WHERE address LIKE '%'||#{address}||'%' " + "AND contenttype=39 " +
	 * "ORDER BY contentid ASC " + "OFFSET #{start} ROWS FETCH NEXT 10 ROWS ONLY")
	 */
	public List<SeoulVO> seoulReserveData(Map map);
	
	/*
	 * @Select("SELECT CEIL(COUNT(*)/10.0) " + "FROM seoultravel " +
	 * "WHERE address LIKE '%'||#{address}||'%' " + "AND contenttype=39")
	 */
	 ...
	 */
	@ResultMap("resMap")
	@Select("SELECT r.no,cno,rday,rtime,rinwon,TO_CHAR(regdate,'') as dbday,isReserve,title,image1,address "
